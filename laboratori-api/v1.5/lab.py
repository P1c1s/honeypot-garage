...
for router in rete:
    if router == "nat":
        lab.new_machine(router, image = get_image_for_host(image_host, router), privileged=True)
    else:
        lab.new_machine(router, image = get_image_for_host(image_host, router), privileged=False)
    radvd_lines = [""]

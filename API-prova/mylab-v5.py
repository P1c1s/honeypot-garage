...
for router in rete:
    ...
    startup_lines = ["sysctl -w net.ipv6.conf.all.forwarding=1\n"]
    radvd_lines = []
    ...
    for i in range(len(rete[router]["lan"])) :          # creazione degli switch per ogni zampa lan del router
        ...
        for host in rete[router]["switch"][switch] :    # creazione degli host connessi alle zampe degli switch
            ...
            lab.create_file_from_list([f"ip address add {str(ip)}/64 dev eth0\nip -6 route add default via {network_address[rete[router]["lan"][i]].replace("::/64", "::1")}"], f"{host}.startup")
...
for router in rete:
    ...
    for r in rete[router].get("route", []):
        ...
        mac = lab.get_machine(next_hop_router).interfaces[rete[next_hop_router]["piface"][0]].mac_address
